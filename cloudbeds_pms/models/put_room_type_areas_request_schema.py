...
class PutRoomTypeAreasRequestSchema(BaseModel):
    """
    PutRoomTypeAreasRequestSchema
    """ # noqa: E501
    layout_type_code: Optional[StrictStr] = Field(description="Layout type: SINGLE (1 area) or MULTI (2+ areas). Set to null to clear all areas configuration.", alias="layoutTypeCode")
    areas: List[AreaItemSchema] = Field(description="List of areas for this room type. Must be empty when layoutTypeCode is null. Must have at least 1 area when layoutTypeCode is not null.")
    __properties: ClassVar[List[str]] = ["layoutTypeCode", "areas"]

    @field_validator('layout_type_code')
    def layout_type_code_validate_enum(cls, value):
        """Validates the enum, returning unknown_default_open_api for unrecognized values"""
        if value is None:
            return value

        _allowed_values = set(['SINGLE', 'MULTI', 'unknown_default_open_api'])
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.areas:
            ...
            _dict['areas'] = _items
        # set to None if layout_type_code (nullable) is None
        # and model_fields_set contains the field
        if self.layout_type_code is None and "layout_type_code" in self.model_fields_set:
            _dict['layoutTypeCode'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "layoutTypeCode": obj.get("layoutTypeCode"),
            "areas": [AreaItemSchema.from_dict(_item) for _item in obj["areas"]] if obj.get("areas") is not None else None
        })
        return _obj
