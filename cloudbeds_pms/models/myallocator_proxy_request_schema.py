...
class MyallocatorProxyRequestSchema(BaseModel):
    """
    MyallocatorProxyRequestSchema
    """ # noqa: E501
    endpoint: StrictStr = Field(description="MyAllocator endpoint name to proxy the request to.")
    payload: Dict[str, Any] = Field(description="Request payload to forward to MyAllocator. Opaque JSON body.")
    __properties: ClassVar[List[str]] = ["endpoint", "payload"]

    @field_validator('endpoint')
    def endpoint_validate_enum(cls, value):
        """Validates the enum, returning unknown_default_open_api for unrecognized values"""
        _allowed_values = set(['RoomStaticContentSave', 'RoomUpdate', 'unknown_default_open_api'])
